...
def fit(X, y, M=10, C=None, instance_categorization=False):
    ...
    # W_ada = methods.intinitialization_weight_adjustment(N)
    # Create list of each models svm after adaboost
    w = []
    b = []
    # create list of confident
    alpha = []
    if instance_categorization is True:
        ...
        for i in range(M):
            ...
            # C_ada = methods.update_instance_categorization_2(X, y, wi, bi)
            # C_ada = methods.update_instance_categorization_3(X, y, wi, bi)
            C_ada = methods.update_instance_categorization_final(X, y, wi, bi)
    else:
        for i in range(M):
            # Creat model
